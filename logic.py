...
class RoboflowLogic:
    ...
    def noisyFilter(self,obj, mean=0, var=0.01):
        ...
        # cv2.imshow("1",self.myLogicObject.noisyFilter(self.myLogicObject.Data[0])[0])
        cv2.imshow("test",noised )
        cv2.imwrite(os.path.join(self.output_path , obj.name+".jpg"), noised)
        print("asfd")
        return  os.path.join(self.output_path , obj.name+".jpg")
